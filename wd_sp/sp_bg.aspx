<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="sp_bg.aspx.cs" Inherits="MarketInfo.wd_sp.sp_bg" %>
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
                        ...
                        <h3 class="header smaller lighter blue"> <a class="btn btn-info" onclick="go_shenpi()"><i class="ace-icon fa fa-reply icon-only"></i>&nbsp;返回</a>
                         <a class="btn btn-info" href="sp_bghis.aspx">
                          &nbsp;历史记录</a>
                        </h3>
						<div class="row">                        
                                            ...
                                            <%if (tb != null)
												...
												<tr>
													...
													<td class="input-group-addon hidden-480"><%=tb.Rows[i]["zname"]%></td>
													  <td class="input-group-addon">
														<span style="color:blue" onclick="f_OpenDlg('<%=tb.Rows[i]["id"]%>')" title="<%=tb.Rows[i]["reason"].ToString() %>">
													...
													<td class="input-group-addon Hide_types">
														<div class="hidden-sm hidden-xs btn-group">															
															<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["id"]%>','<%=tb.Rows[i]["tid"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
														</div>
														<div class="hidden-md hidden-lg">
															<div class="inline pos-rel">
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	<li>
																		<a href="#" class="tooltip-info"  onclick="f_shenpi('<%=tb.Rows[i]["id"]%>','<%=tb.Rows[i]["tid"]%>','-1')" data-rel="tooltip" title="View">
																			<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["id"]%>','<%=tb.Rows[i]["tid"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
																		</a>
...
<![endif]-->
        ...
        <script type="text/javascript">
            ...
            function f_shenpi(id,sid, type) {
                mui.confirm('是否同意？', '审批', ['同意', '退回', '取消'], function (e) {
                   // alert(e.index);
                    if (e.index == 1) {
                        f_set(id,sid, type);
                    }
            ...
            function f_set(id,tid, type) {               
                    f_UpTitle(id,tid, type);
              
               // alert(type);
               
            }
            function f_UpTitle(id,tid, type) {
                ...
                layer.open({
                   ...
                   // content: '<div id="mydiv" style="margin-left:10px;margin-top:10px;"><input id="txtReason" type="text" value="' + title + '" autocomplete="off" /><input rows="3" cols="0" class="form-control input-mask-phone" id="txtreson" placeholder="请输入审批建议" /></div>',
                    ...
                    , yes: function (index, layero) {
                        ...
                        //  alert(id);
                        //type是否同意变更
                        $.post("../it/SP_BG.ashx", { TYPE: type,ID: id, TID: tid, Reason: txtReason }, function (data) {
                            if (data == "1") {
                                mui.toast("审批成功!");
                                location.reload();
                            }
	...
	<!-- inline scripts related to this page -->
        ...
        location.href = "../wd_shenpi/wd_shenpinew.aspx?TID=" + HideTid;
    }
		</script>
	</body>
</html>
