...
namespace MarketInfo.wd_tongji
{
    ...
    /// canguan 的摘要说明
    /// </summary>
    public class canguan : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (strtype == "2")//站长
            {
                ...
                StrSql = "select  count(*) as num from dbo.T_WORK_INFO1 where " + where + " and T_TYPEID='3' and DATEDIFF (S, T_WSTART,'" + strnowdate + "')  >= 0 and DATEDIFF(S,T_WEND,'" + strnowdate + "' )<=0 ";
            ...
            if (strtype == "0")//区主任
            {
                StrSql = "select  count(*) as num from dbo.T_WORK_INFO1 where T_TYPEID='3' and DATEDIFF (S, T_WSTART,'" + strnowdate + "')  >= 0 and DATEDIFF(S,T_WEND,'" + strnowdate + "' )<=0 ";
            }
            if (strtype == "3")//线路管理员
            {
                StrJson = "-1";
