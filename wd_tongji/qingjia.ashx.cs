...
namespace MarketInfo.wd_tongji
{
    ...
    /// qingjia 的摘要说明
    /// </summary>
    public class qingjia : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (strtype == "2")//站长
            {
                ...
                //StrSql = "select  count(*) as num from dbo.T_WORK_INFO1 where " + where + " and T_TYPEID='1' and DateDiff(dd,T_CRTIME ,getdate())=0";
                //DATEDIFF (S, T_WSTART,'" + strnowdate + "')  > 0 and DATEDIFF(S,T_WEND,'" + strnowdate + "' )<0 
                StrSql = "select  count(*) as num from dbo.T_WORK_INFO1 where " + where + " and T_TYPEID='1' and DATEDIFF (S, T_WSTART,'" + strnowdate + "')  >= 0 and DATEDIFF(S,T_WEND,'" + strnowdate + "' )<=0 ";
            ...
            if (strtype == "0")//管理处
            {
                ...
                StrSql = "select  count(*) as num from dbo.T_WORK_INFO1 where T_TYPEID='1' and DATEDIFF (S, T_WSTART,'" + strnowdate + "')  >= 0 and DATEDIFF(S,T_WEND,'" + strnowdate + "' )<=0 ";
            }
            if (strtype == "3")//线路管理员
            {
