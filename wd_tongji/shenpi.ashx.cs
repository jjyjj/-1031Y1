...
namespace MarketInfo.wd_tongji
{
    ...
    /// shenpi 的摘要说明
    /// </summary>
    public class shenpi : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (strtype == "2")//站长
            {
                ...
                // StrSql = "select  count(*) as num from T_File where " + where + " ";
                StrSql = "SELECT (select count(1) total from T_File   where " + where + ")+(select count(1) total from T_File   where " + where + ") AS num";
            }
            if (strtype == "1")//区主任
            {
            ...
            if (strtype == "0")//区主任
            {
                ...
                StrSql = "SELECT (select count(1) total from T_File_1 )+(select count(1) total from T_File) AS num ";
            }
            if (strtype == "3")//线路管理员
            {
