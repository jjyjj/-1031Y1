...
namespace MarketInfo.wd_dzrz.wd_disanf
{
    ...
    /// seldsfsg 的摘要说明
    /// </summary>
    public class seldsfsg : IHttpHandler, IRequiresSessionState 
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string StrSql = "select  * from dbo.T_Dsan_shigong where t_id='" + strtid + "'  order by id desc";
            if (!string.IsNullOrWhiteSpace(strid))
            {
                StrSql = "select  * from dbo.T_Dsan_shigong where  id=" + strid;
            }
            try
            {
