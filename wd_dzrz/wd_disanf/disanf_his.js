...
function xzsgData() {
    var id = $('#Hidtid').val();
    $.post('his_it/selxzsg_his.ashx', { tid: id }, function (data) {
        if (data != "-1") {
            xzsgBind(data);
        } else {
            mui.toast('数据读取失败');
...
function xzsgBind(data) {
    ...
    for (var p in data) {
        xiangqing = '<a href="javascript:;"  title="详情" onclick="xzsgdata(' + data[p].id + ')"><i class="ace-icon glyphicon glyphicon-file"></i></a>';
        if (data[p].sp1 == '-1' || data[p].sp2 == '-1' || data[p].sp3 == '-1') {
            td1 = '<td class="input-group-addon" style="position:relative;">' + shenpi + (parseInt(p) + 1) + ' </td><td class="input-group-addon">' + data[p].xinxiqudao + '</td>';
        } else {
...
function add_dsfsg() {
    ...
    $.post('adddsfsg.ashx', { zh: zh, zczyq: zczyq, xzqyzh: xzqyzh, gcmc: gcmc, ms: ms, lxfs: lxfs, gdglsm: gdglsm, startime: startime, sgjd: sgjd,
...
function update_dsfsg(id) {
    ...
    $.post('updsfsg.ashx', { zczyq: zczyq, zh: zh, xzqyzh: xzqyzh, gcmc: gcmc, ms: ms, lxfs: lxfs, gdglsm: gdglsm, startime: startime, sgjd: sgjd, id: id,
        is_hfff_jc: is_hfff_jc, gdms: gdms, sg_isns: sg_isns, pshjl: pshjl, is_jd: is_jd, bzsg: bzsg, xzsg: xzsg, zcfzr: zcfzr, remark: remark
    }, function (data) {
        if (data == '0') {
            mui.toast('修改成功');
...
function dsfsgData() {
    ...
    $.post('his_it/seldsfsg_his.ashx', { tid: id }, function (data) {
        if (data != "-1") {
            dsfsgBind(data);
        } else {
            mui.toast('数据读取失败');
...
function dsfsgBind(data) {
    ...
    var shenpi = '<i class="ace-icon fa fa-exclamation-triangle bigger-121" style="  cursor:pointer; position:absolute; left:15px; top:11px; color:#D15B47; "  title="审批未通过/被退回/修改后可再次审批"></i> ';
    for (var p in data) {
        caozuo = '<a href="javascript:;"  title="详情" onclick="dsfsgdata(' + data[p].id + ')"><i class="ace-icon glyphicon glyphicon-file"></i></a>';
        if (data[p].sp1 == '-1' || data[p].sp2 == '-1' || data[p].sp3 == '-1') {
            td1 = '<td class="input-group-addon" style="position:relative;">' + shenpi + (parseInt(p) + 1) + '</td><td class="input-group-addon">' + data[p].gcmc + '</td>';
        } else {
...
function jhpDaTa() {
    var id = $('#Hidtid').val();
    $.post('his_it/seljhd_his.ashx', {tid:id}, function (data) {
        if (data != "-1") {
            jhpBind(data);
        } else {
            mui.toast('数据读取失败');
...
function jhpBind(data) {
    ...
    var shenpi = '<i class="ace-icon fa fa-exclamation-triangle bigger-121" style="  cursor:pointer; position:absolute; left:15px; top:11px; color:#D15B47; "  title="审批未通过/被退回/修改后可再次审批"></i> ';
    for (var p in data) {
        caozuo = '<a href="javascript:;"  title="查看" onclick="jhdupdate(' + data[p].id + ')"><i class="ace-icon fa fa-pencil-square-o"></i></a>';
        if (data[p].sp1 == '-1' || data[p].sp2 == '-1' || data[p].sp3 == '-1') {
            td1 = '<td class="input-group-addon" style="position:relative;">' + shenpi + (parseInt(p) + 1) + '</td><td class="input-group-addon">' + data[p].jhd + '</td><td class="input-group-addon">' + data[p].jhname + '</td>';
        } else {
...
function jcxmDaTa() {
    var id = $('#Hidtid').val();
    $.post('his_it/seljc_his.ashx', {tid:id}, function (data) {
        if (data != "-1") {
            jcxmBind(data);
        } else {
            mui.toast('数据读取失败');
...
function jcxmBind(data) {
    ...
    for (var p in data) {
        if (data[p].sp1 == '-1' || data[p].sp2 == '-1' || data[p].sp3 == '-1') {
            td1 = '<td class="OverHideTd" style="position:relative;">' + shenpi + (parseInt(p) + 1) + '</td><td class="OverHideTd">' + data[p].jrjcxm + '</td><td class="OverHideTd">' + data[p].fxwt + '</td>';
        } else {
            td1 = '<td class="OverHideTd">' + (parseInt(p) + 1) + '</td><td class="OverHideTd">' + data[p].jrjcxm + '</td><td class="OverHideTd">' + data[p].fxwt + '</td>';
        }
        caozuo = '<a href="javascript:;"  title="查看" onclick="jcxmupdate(' + data[p].id + ')"><i class="ace-icon fa fa-pencil-square-o"></i></a>';
