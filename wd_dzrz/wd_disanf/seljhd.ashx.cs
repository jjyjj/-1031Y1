...
namespace MarketInfo.wd_dzrz.wd_disanf
{
    ...
    /// seljhd 的摘要说明
    /// </summary>
    public class seljhd : IHttpHandler, IRequiresSessionState 
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string StrSql = "select  * from dbo.T_Dsan_jindu where t_id='" + strtid + "' order by id desc";
            if (!string.IsNullOrWhiteSpace(strid))
            {
                StrSql = "select  * from dbo.T_Dsan_jindu where id=" + strid;
            }
            try
            {
