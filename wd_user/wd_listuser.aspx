<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wd_listuser.aspx.cs" Inherits="WeiXinGIS.wd_user.wd_listuser" %>
...
<title>用户信息列表</title>
    <script type="text/javascript">
        function f_isDel(strname,id) {
            if (confirm("是否删除用户[" + strname + "]?")) {
                location.href = "wd_deluser.aspx?TID=" + id;
            }
        }
    </script>
    </head>
...
<span class="l_f">
    <a href="../wd_dzrz/zyqlist.aspx" title="返回" class="btn btn-warning Order_form"><i class="icon-plus"></i>返回</a>
</span>
       </div>
     <!--分类类表-->
       ...
       <thead>
		 <tr>
             ...
             <th class="input-group-addon">真实姓名</th>
                ...
                <th class="input-group-addon hidden-480" width="10%">修改</th>
                <th class="input-group-addon hidden-480" width="10%">删除</th>
			</tr>
		</thead>
        <tbody>
            <%if (tb != null)
         ...
         <tr>
          ...
          <td class="input-group-addon" nowrap="nowrap" 
          ...
          <td class="input-group-addon" height="24" 
              ...
              <td class="input-group-addon" height="24" 
                 style="width:15%; overflow: hidden; word-break: keep-all; cursor: pointer; " 
         ...
         <td class="input-group-addon hidden-480"><a href="wd_newuser.aspx?T_UNAME=<%=tb.Rows[i]["T_UNAME"]%>&T_PWD=<%=tb.Rows[i]["T_PWD"]%>">
                    <img alt="" class="style1" src="images/f03.png" 
                        style="border-width: 0px; cursor: pointer" /></a></td>
         <td class="input-group-addon hidden-480" width="10%"><img alt="" onclick="f_isDel('<%=tb.Rows[i]["T_UNAME"]%>','<%=tb.Rows[i]["T_ID"]%>')" class="style2" src="images/close.png" 
