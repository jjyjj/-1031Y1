<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GHGQ.aspx.cs" Inherits="MarketInfo.wd_shenpi.GHGQ" %>
				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="page-header">
                         ...
                         <a class="btn btn-info" href="../wd_dzrz/wd_ghgqlist_his.aspx?UTID=<%=UTID%>">
                          &nbsp;历史记录</a>
                        </h3>
						<div class="row">
							<div class="col-xs-12">
										...
										<table id="simple-table" class="table  table-bordered table-hover">
											<thead>
												...
												<th class="input-group-addon"><i class="ace-icon fa fa-flag"></i>序号</th> 
													<th class="input-group-addon">高果区编号</th> 
                                                    <th class="input-group-addon hidden-480">今日检查</th> 
                                                    <th class="input-group-addon hidden-480">发现问题</th>                                                     
                                                    <th class="input-group-addon hidden-480">问题跟踪</th> 
                                                    <th class="input-group-addon">宣传走访</th> 
                                                    <th class="input-group-addon hidden-480">应急演练</th> 
                                            ...
                                            <%if (tb != null)
												...
												<tr> 
                                                    ...
                                                    <th class="input-group-addon"><%=tb.Rows[i]["g_ggqbh"]%></th> 
                                                    <th class="input-group-addon hidden-480"><%=tb.Rows[i]["g_jinri_jiancha"]%></th> 
                                                    <th class="input-group-addon hidden-480"><%=tb.Rows[i]["g_faixian_went"]%></th>                                                     
                                                    <th class="input-group-addon hidden-480"><%=tb.Rows[i]["g_wenti_genzong"]%></th> 
                                                    <th class="input-group-addon"><%=tb.Rows[i]["g_xuanchuan_zoufang"]%></th> 
                                                    <th class="input-group-addon hidden-480"><%=tb.Rows[i]["g_yingji_yanlian"]%></th> 
                                                    <th class="input-group-addon hidden-480"><%=tb.Rows[i]["g_qita"]%></th>                                           
													<td class="input-group-addon Hide_types">
														<div class="hidden-sm hidden-xs btn-group">															
															<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["g_id"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
														</div>
														<div class="hidden-md hidden-lg">
															<div class="inline pos-rel">
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	<li>
																		<a href="#" class="tooltip-info"  onclick="f_shenpi('<%=tb.Rows[i]["g_id"]%>','-1')" data-rel="tooltip" title="View">
																			<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["g_id"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
																		</a>
...
//                $.post("../it/GHGSP.ashx", { TYPE: type, ID: id, sp: Hide_Sp }, function (data) {
//                    if (data == "1") {
//                        mui.toast("审批成功!");
//                        location.reload();
//                    }
            ...
            function f_UpTitle(id, type) {
                ...
                layer.open({
                    ...
                    , yes: function (index, layero) {
                        ...
                        //}
                        var Hide_Sp = $('#Hide_Sp').val();
                        $.post("../it/GHGSP.ashx", { TYPE: type, ID: id, sp: Hide_Sp, Reason: txtReason }, function (data) {
                            if (data == "1") {
                                mui.toast("审批成功!");
                                location.reload();
                            }
