<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GDGL.aspx.cs" Inherits="MarketInfo.wd_shenpi.GDGL" %>
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="page-content">
                        ...
                        <a class="btn btn-info" href="../wd_dzrz/wd_guandao_fc_his.aspx?UTID=<%=UTID%>">
                          &nbsp;历史记录</a>
                        </h3>
						<div class="row">
							<div class="col-xs-12">
										...
										<table id="simple-table" class="table  table-bordered table-hover">
											<thead>
												<tr>
													...
													<th class="input-group-addon">辖区管道里程</th>
													<th class="input-group-addon hidden-480">辖区光缆里程</th> 
													<th class="input-group-addon hidden-480">今日复测管道里程</th>  
													<th class="input-group-addon hidden-480">今日复测光缆里程</th>
													<th class="input-group-addon hidden-480">发现问题</th> 
                                                    <th class="input-group-addon hidden-480">问题整改</th>
													<th class="input-group-addon hidden-480">防护措施</th>
                                                    <th class="input-group-addon hidden-480">累积复测里程</th>
                                            ...
                                            <%if (tb != null)
												...
												<tr>                                              
													...
													<td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_xiaqu_gllc"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_jinri_fcgdlc"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_jinri_fcgllc"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_faxian_wt"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_fuce_lc"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_fuce_jd"]%></td>
                                                    <td class="input-group-addon hidden-480"><%=tb.Rows[i]["gd_wt"]%></td>
                                                    <td class="input-group-addon"><span class="label label-sm label-warning"><%=tb.Rows[i]["gd_fh"]%></span></td> 
													<td class="input-group-addon Hide_types">
														<div class="hidden-sm hidden-xs btn-group">															
															<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["gd_id"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
														</div>
														<div class="hidden-md hidden-lg">
															<div class="inline pos-rel">
																...
																<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																	<li>
																		<a href="#" class="tooltip-info"  onclick="f_shenpi('<%=tb.Rows[i]["gd_id"]%>','-1')" data-rel="tooltip" title="View">
																			<button class="btn btn-xs btn-danger">
																<span onclick="f_shenpi('<%=tb.Rows[i]["gd_id"]%>','-1')"><i class="ace-icon fa fa-flag bigger-120"></i></span>
															</button>
																		</a>
...
//                $.post("../it/GDSP.ashx", { TYPE: type, ID: id, sp: Hide_Sp }, function (data) {
//                    if (data == "1") {
//                        mui.toast("审批成功!");
//                        location.reload();
//                    }
            ...
            function f_UpTitle(id, type) {
                ...
                layer.open({
                    ...
                    , yes: function (index, layero) {
                        ...
                        //}
                        var Hide_Sp = $('#Hide_Sp').val();
                        $.post("../it/GDSP.ashx", { TYPE: type, ID: id, sp: Hide_Sp, Reason: txtReason }, function (data) {
                            if (data == "1") {
                                mui.toast("审批成功!");
                                location.reload();
                            }
