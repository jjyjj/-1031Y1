...
(function (doc, win) {
 ...
 function f_Init() {
     objMapFrm = document.getElementById("IFRM_MAP").contentWindow;
 }

 function f_Move2Pos(arrPos) {
     //alert(arrPos);
     objMapFrm.MoveTo(arrPos[0], arrPos[1],1000,0.0,-90.0);
 }

 function f_ShowLyr(chk,lyrObj) {
     lyrObj.show = chk;
     //alert(chk);
 }

 function f_ShowDEM(chk) {
     objMapFrm.f_ShowDEM(chk);
 }

 function f_GetHighWay() {
     var strURL = "mapHighway/it.aspx?CMD=GS";
     $.post(strURL, null, function (data) {
         if (data == "") return;
         //alert(data);
         objMapFrm.f_LoadJsonMap(data); 
          });
     objMapFrm.f_LoadJsonMap("../mapHighway/it.aspx?CMD=SHP"); /// <reference path="../mapHighway/it.aspx" />
 ...
 function f_GetCityPos() {
    var strURL = "mapHighway/it.aspx?CMD=GS";
    $.post(strURL, null, function (data) {
        if (data == "") return;
        var arrCityS = eval("(" + data + ")");
        if (arrCityS.length != 0) {
            for (var i = 0; i < arrCityS.length; i++) {
                //XID,XNAME,TYPE,GEOM
                var gid = arrCityS[i].gid;
                var name = arrCityS[i].name;
                var code = arrCityS[i].code;
                var mktxy = _GetWKTXY(arrCityS[i].xy); //返回的是MKT格式坐标
                //objMapFrm.f_AddPoint(name, mktxy[0], mktxy[1], 5, "CITY", 12);/// <reference path="../images/gdline/xy.png" />

                objMapFrm.f_AddMark(name, mktxy[0], mktxy[1], 5, "CITY", 12, "../images/gdline/xy.png", 14);
            }
        }
    });
}

function _GetWKTXY(geom) {
    //POINT(113.619319024448 34.7483628259695)
