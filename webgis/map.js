...
//var strpcxq = "http://192.168.0.200:10/mktxyz.aspx?T=HENANMKTGIS&z={z}&x={x}&y={y}";
//var strpcxq = "http://t4.tianditu.com/DataServer?T=vec_w&x={x}&y={y}&l={z}&tk=221b586ea9d6fe72785d8b56c61aa396";
var strpcxq = 'http://sft.shuzihenan.com/mktxyz.aspx?T=SFTMKT&z={z}&x={x}&y={y}';
...
var strtdtzj = "http://t4.tianditu.com/DataServer?T=cva_w&x={x}&y={y}&l={z}&tk=221b586ea9d6fe72785d8b56c61aa396";
var map_tdtzj = new ol.layer.Tile({
    source: new ol.source.XYZ({
        url: strtdtzj
    }),
    projection: 'EPSG:3857'
    ...
    if (cur_Tool == 2) {
        ...
        if (feature == undefined) {
            ...
            try {
                ...
                if (ic == "90") {
                    try { parent.f_GetLKByGID(gid); } catch (e) { }
                    try { parent.f_ShowDlg(gid); } catch (e) { }                    
                }
            }
            catch (e) {
...
//新增一个添加矢量Line的函数
function map_AddPolyLine(gid, name, wkt_geom, m_class, vectorlyr, iColor, iLineWid) {
    //iColor='rgba(255, 0, 0, 1)'
    var format = new ol.format.WKT();
...
function AddCircle(centerPoint,radius,objLayer) {// 
    //var centerradius = new ol.geom.Circle([12670926.799,4023424.989],1000);
    //centerradius = centerradius.clone().transform('EPSG:4326', 'EPSG:3857');
    var centerradius = new ol.geom.Circle(centerPoint, radius);
    var circlefeature = new ol.Feature({
        geometry: centerradius,
        name: ''
    });
    objLayer.getSource().addFeature(circlefeature);
}
...
function f_FwRenShu() {
    ...
    $.post(strURL, null, function (data) {
        ...
        var strtitle = "今日服务：" + allnum.toLocaleString() + "人次";
